...
- Per-blog: dynamically generated by `/src/pages/og/[slug].svg.ts` at build time. Pulls post title (auto-wrapped to 3 lines), category label, date, read-time, renders in brand cover style.
- `Layout.astro` resolves `ogImage` to absolute URL, sets `og:image:type` (auto-detects SVG vs JPEG), `og:image:width=1200`, `og:image:height=630`, `og:image:alt`, `twitter:card`, `twitter:site`, `twitter:creator`.
...
4. Build output: 19 static pages + 6 dynamic OG SVGs + 3 serverless functions (`/api/contact`, `/api/subscribe`, `/api/resource-gate`)
