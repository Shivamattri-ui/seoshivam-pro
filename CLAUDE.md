...
### URL trailing-slash convention (LOCKED)
One canonical URL form site-wide: **leaf pages have NO trailing slash, the homepage root keeps its `/`.** So `https://seoshivam.pro/` but `https://seoshivam.pro/insights/slug` (no slash). This is enforced by `trailingSlash: 'never'` in `astro.config.mjs`, which also makes the Vercel adapter emit a **308 redirect** `^/(.*)/$` → `/$1` (so `/slug/` redirects to `/slug`; the root never matches, so it isn't redirected). `<link rel=canonical>` and `og:url` come from `Astro.url.href` and already match this. Internal links must be written **without** trailing slashes (a slash form just wastes a 308).
- **Why it matters:** sitemap `<loc>`, canonical, og:url, and the served URL must all be the *same string* per page. Before this fix, subpages served 200 at BOTH `/slug` and `/slug/` while the sitemap listed the slash form and the canonical listed the no-slash form: duplicate URLs + contradictory signals. Don't reintroduce that.
- **Root-slash caveat:** under `trailingSlash: 'never'`, `@astrojs/sitemap` hard-strips the homepage `<loc>` to `https://seoshivam.pro` (see its `write-sitemap.js`), which would mismatch the homepage canonical `https://seoshivam.pro/`. `scripts/fix-sitemap-root.mjs` (wired first in the `postbuild` chain, before IndexNow) re-adds that single root slash in both `dist/client` and `.vercel/output/static`. Keep it.
- **Sitemap** (`@astrojs/sitemap`) emits per-URL `<lastmod>`: blog posts get their `updatedDate || pubDate` (mapped in `astro.config.mjs` via `blogLastmod()` + the `serialize` hook), other pages get build time. This is the freshness signal for Google. The same `serialize` hook also strips any non-root trailing slash as a guard. Don't drop the serialize hook.
- **IndexNow** (`scripts/indexnow-submit.mjs`, wired as `postbuild`): on every Vercel **production** build it POSTs all sitemap URLs to `api.indexnow.org`, instantly notifying Bing/Yandex/Naver/Seznam. Bing's index powers ChatGPT Search + Copilot, so this is the main "instant indexing" lever. Google does NOT use IndexNow.
  - Public key file: `public/557bee82cbe04cc4835072b085e3edbc.txt` (this is the key **generated by Bing Webmaster Tools** for the verified site; public by design, served at that URL; `keyLocation` + the `KEY` constant in the script must keep matching this filename).
  ...
  - The site is verified in **Bing Webmaster Tools** and the key above is BWT's own IndexNow key, so submissions are authorized. (Before BWT verification, IndexNow returns `403 UserForbiddedToAccessSite` regardless of a correct key file: Bing must recognize domain ownership first.) Verify submissions in **Bing Webmaster Tools → IndexNow**.
