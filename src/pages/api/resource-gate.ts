...
export const POST: APIRoute = async ({ request }) => {
    ...
    return json(
      {
        ok: false,
        error:
          'BREVO_API_KEY is not set. Local: create a .env file in the project root with BREVO_API_KEY=your_key and restart npm run dev. Production: add BREVO_API_KEY in Vercel → Project → Settings → Environment Variables, then redeploy.',
      },
      500,
    );
  }
  ...
  // Only attributes that already exist on list-3 contacts (same pattern as subscribe).
  // Brevo rejects unknown keys; segment on LEAD_SOURCE in the dashboard.
  ...
  try {
    ...
    if (res.ok || res.status === 204) {
      return json({ ok: true, message: 'You are on the list. Check your inbox for the file.' });
    }

    // Duplicate contact: Brevo often returns 400 with duplicate_identifier in body
